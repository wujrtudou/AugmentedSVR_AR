...
% the temporal lag P
TemporalUInputTrain=BasicU_Series(1:end-1);
TemporalXInputTrain=InputTrain(2:end,:);
TemporalYOutTrain=OutputTrain(2:end);
...
NewTemporalUInputTrain=NewU_Series(1:end-1);
...
Temporal_RMSE=sqrt(mean((TemporalPrediction-TrueResponse).^2))
Basic_RMSE=sqrt(mean((BasicPrediction-TrueResponse).^2))
figure(1)
plot(TrueResponse)
hold on
